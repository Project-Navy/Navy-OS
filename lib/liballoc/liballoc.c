...
#define VERSION 	"1.1"
#define ALIGNMENT	16ul        // 4ul ///< This is the byte alignment that memory must
                                // be allocated on. IMPORTANT for GTK and other stuff.

#define ALIGN_TYPE		char    // /unsigned char[16] /// unsigned short
#define ALIGN_INFO		sizeof(ALIGN_TYPE)*16   // /< Alignment information is stored
...
struct liballoc_major
{
    ...
    struct liballoc_minor *first;   // /< A pointer to the first allocated memory in the
                                    // block. 
};
...
static struct liballoc_major *l_memRoot = NULL; // /< The root memory block acquired from 
                                                // the system.
static struct liballoc_major *l_bestBet = NULL; // /< The major with the most free
                                                // memory.

static unsigned int l_pageSize = 4096;  // /< The size of an individual page. Set up in
                                        // liballoc_init.
static unsigned int l_pageCount = 16;   // /< The number of pages to request per chunk.
                                        // Set up in liballoc_init.
...
static struct liballoc_major *
allocate_new_page(unsigned int size)
{
    unsigned int st;
...
void *
liballoc_malloc(size_t req_size)
{
    int startedBet = 0;
...
#ifdef DEBUG
    printf("liballoc: %x liballoc_malloc( %i ): ", __builtin_return_address(0), size);
    FLUSH();
#endif
